...
class PgyerTask extends DefaultTask {
    ...
    private HashMap<String, JSONObject> httpPost(String endPoint, List<Apk> apks) {
        ...
        for (Apk apk in apks) {
            ...
            println "apk file : " + apk.file
            RequestBody multiParams = RequestBody.create(MediaType.parse("application/vnd.android.package-archive"), apk.file);
            multipartBuilder.addFormDataPart("file", apk.file.name, multiParams)
